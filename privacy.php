...
'en'=>[
 ...
 'verify'=>'When a reference is checked, the entered verification code is processed to display the status of an audit record. To protect against automated or abusive queries, the IP address may be pseudonymously hashed with a server-side secret and the resulting hash stored briefly with the timestamp for rate limiting; these records are removed after no more than 24 hours. Public results contain only information approved for publication. Confidential client or project data is not disclosed.',
