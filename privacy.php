...
<div class="definition"><strong>1. Verantwortlicher</strong><div><?= mmEscape((string)($legal['brand'] ?? 'MysteryMarket')) ?> · <?= mmEscape((string)($legal['owner_name'] ?? '')) ?><br><?= mmEscape((string)($legal['street'] ?? '')) ?>, <?= mmEscape(trim((string)($legal['postal_code'] ?? '') . ' ' . (string)($legal['city'] ?? ''))) ?><br><a href="mailto:<?= mmEscape((string)($legal['email'] ?? 'robert.breuss@mysterymarket.de')) ?>"><?= mmEscape((string)($legal['email'] ?? 'robert.breuss@mysterymarket.de')) ?></a></div></div>
...
<div class="definition"><strong>8. Ihre Rechte</strong><div>Sie haben im Rahmen der gesetzlichen Voraussetzungen insbesondere Rechte auf Auskunft, Berichtigung, Löschung, Einschränkung der Verarbeitung, Datenübertragbarkeit und Widerspruch. Außerdem besteht ein Beschwerderecht bei einer Datenschutzaufsichtsbehörde.</div></div>
